...
export class DuckDBServiceSandboxed {
  ...
  static getInstance(logger?: Logger, errorHandler?: ErrorHandler): DuckDBServiceSandboxed {
    if (!DuckDBServiceSandboxed.instance) {
      ...
      DuckDBServiceSandboxed.instance = new DuckDBServiceSandboxed(logger, errorHandler);
    }
  ...
  async duckdbCommand(command: string, data: any): Promise<any> {
    try {
      ...
      const dataInfo = data ? (
        command === 'query' ? `embedding[${data.embedding?.length || 0}], topK=${data.topK || 5}` :
        command === 'save' ? `vectors[${data.vectors?.length || 0}]` :
        'with data'
      ) : 'no data';
      ...
      const resultInfo = result ? (
        command === 'query' && result.matches ? `${result.matches.length} matches` :
        command === 'save' && result.success ? 'saved successfully' :
        'completed'
      ) : 'no result';
  ...
  private getFallbackResponse(command: string): any {
    switch (command) {
      case 'query':
        return { matches: [] };
      case 'save':
        return { success: false, error: 'DuckDB not available in sandboxed mode' };
      case 'count':
        return { count: 0 };
      case 'debug':
        return { 
  ...
  async initialize(): Promise<void> {
    this.logger.info('DuckDB Sandboxed Service initialized - operations via IPC');
  }
