...
export class MemoryContextBuilder implements IMemoryContextBuilder {
  ...
  async fetchContextualMemory(
    ...
    try {
      ...
      const effectiveTemporaryContext =
        temporaryContext !== undefined
          ? temporaryContext
          : this.contextManager.getTemporaryContext();

      // Check if we have a non-empty temporary context after normalization (context integrity check)
      if (
        effectiveTemporaryContext &&
        effectiveTemporaryContext.trim().length > 0
      ) {
        // Check if the context has changed since the last query (context drift detection)
        const contextChanged = this.contextManager.hasTemporaryContextChanged(
          effectiveTemporaryContext
        );
        ...
        if (contextChanged) {
          ...
          LoggingUtils.logInfo(
            `[COGNITIVE-CONTEXT] Querying Pinecone for new temporary context: ${effectiveTemporaryContext.substring(
              0,
              30
            )}...`
          );
          result.temporaryContext = await this.queryExternalMemory(
            effectiveTemporaryContext,
            topK,
            keywords
          );
          ...
          this.contextManager.updateLastQueriedTemporaryContext(
            effectiveTemporaryContext
          );

          // Store the retrieved context memory in the contextManager (neural memory cache)
          if (result.temporaryContext) {
            this.contextManager.setTemporaryContextMemory(
              result.temporaryContext
            );
            LoggingUtils.logInfo(
              `[COGNITIVE-CONTEXT] Temporary context retrieved: ${(
                result.temporaryContext ?? ""
              ).substring(0, 50)}...`
            );
          }
          ...
          result.temporaryContext =
            this.contextManager.getTemporaryContextMemory();
          if (
            !result.temporaryContext ||
            result.temporaryContext.trim() === ""
          ) {
            LoggingUtils.logInfo(
              `[COGNITIVE-CONTEXT] No temporary context found in cache for: ${(
                effectiveTemporaryContext ?? ""
              ).substring(0, 50)}...`
            );
          } else {
            LoggingUtils.logInfo(
              `[COGNITIVE-CONTEXT] Using cached temporary context (no neural query)`
            );
          }
        }
      }
      ...
      if (userTranscriptions.length > 0) {
        ...
        if (userTranscriptText.trim()) {
          const userContext = await this.queryExternalMemory(
            userTranscriptText,
            topK,
            keywords
          );
          if (!userContext || userContext.trim() === "") {
            LoggingUtils.logInfo(
              `[COGNITIVE-CONTEXT] No context found for user input: ${(
                userTranscriptText ?? ""
              ).substring(0, 50)}...`
            );
          } else {
            LoggingUtils.logInfo(
              `[COGNITIVE-CONTEXT] User context retrieved: ${(
                userTranscriptText ?? ""
              ).substring(0, 50)}...`
            );
          }
      ...
      if (detectedSpeakers.has("external")) {
        if (externalTranscriptions.length > 0) {
          ...
          if (externalText.trim()) {
            const externalContext = await this.queryExternalMemory(
              externalText,
              topK,
              keywords
            );
            ...
            if (!externalContext || externalContext.trim() === "") {
              LoggingUtils.logInfo(
                `[COGNITIVE-CONTEXT] No context found for external speaker input: ${(
                  externalText ?? ""
                ).substring(0, 50)}...`
              );
            } else {
              LoggingUtils.logInfo(
                `[COGNITIVE-CONTEXT] External context retrieved: ${(
                  externalText ?? ""
                ).substring(0, 50)}...`
              );
            }
      ...
      LoggingUtils.logError(
        "[COGNITIVE-CONTEXT] Error fetching speaker contexts",
        error
      );
      return result;
    }
  ...
  async queryExternalMemory(
    ...
    try {
      ...
      LoggingUtils.logError(
        "[COGNITIVE-CONTEXT] Error querying external context memory",
        error
      );
      return "";
    }
  ...
  buildMessagesWithContext(
    ...
    const persistentTemporaryContext =
      this.contextManager.getTemporaryContext();
    ...
    const systemMessage =
      conversationHistory.length > 0 ? [conversationHistory[0]] : [];
    const messages: Message[] = [...systemMessage];

    // Add instructions (temporary cognitive context)
    this.addInstructionsToMessages(
      messages,
      persistentTemporaryContext,
      memoryResults
    );
    ...
    useSimplifiedHistory && hasMemoryContext
      ? this.addSimplifiedTranscriptions(
          messages,
  ...
  private addInstructionsToMessages(
    ...
    // Add instructions with proper theoretical framing
    const instructionsContext = `COGNITIVE CONTEXT MODULATION:

THEORETICAL BASIS: Working Memory and Executive Control (Baddeley & Hitch model)
These instructions temporarily modulate your cognitive processing within the current interaction.

CONTEXTUAL PARAMETERS:
${temporaryContext}

INTEGRATION: Apply these parameters while maintaining coherence with your core cognitive architecture.`;
    ...
    // Add memory-linked instructions if available
    const memoryTemporaryContext =
      memoryResults?.temporaryContext ||
      this.contextManager.getTemporaryContextMemory();
    ...
    if (memoryTemporaryContext) {
      messages.push({
        role: "system",
        content: `EPISODIC MEMORY ACTIVATION:

THEORETICAL BASIS: Context-dependent memory retrieval (Tulving's encoding specificity)
Previous contextual parameters have been retrieved from episodic memory.

RETRIEVED CONTEXT:
${memoryTemporaryContext}

INTEGRATION: These retrieved parameters may inform current processing if relevant to the present context.`,
      });
    }
  }
  ...
  private addMemoryContextToMessages(
    ...
    if (
      memoryResults.speakerContexts &&
      memoryResults.speakerContexts.size > 0
    ) {
    ...
    if (externalContext.trim()) {
      ...
      const formattedExternalContext =
        this.formatter.formatExternalSpeakerContent(externalContext);
  ...
  private addSimplifiedTranscriptions(
    ...
    if (lastUserMessage) {
      ...
      const filteredUserContent =
        this.snapshotTracker.filterTranscription(userContent);
    ...
    if (lastExternalMessage) {
      ...
      const originalLabel = lastExternalMessage.text.includes("[")
        ? lastExternalMessage.text.match(/^\[([^\]]+)\]/)?.[1]
        : null;
      ...
      const filteredExternalContent =
        this.snapshotTracker.filterTranscription(externalContent);
  ...
  private addFullTranscriptionsWithDeduplication(
    ...
    const combinedConversation = this.formatter.buildConversationFromSegments(
      segments,
      true
    );
    ...
    if (combinedConversation) {
      finalContent =
        this.snapshotTracker.filterTranscription(combinedConversation);
