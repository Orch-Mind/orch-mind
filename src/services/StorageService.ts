    ...
    return {
      get: () => {
        try {
          ...
          const userName = parsedData[STORAGE_KEYS.USER_NAME] || parsedData['name'] || 'N/A';
      ...
      set: (data: UserSettings) => {
        try {
          ...
          const verification = window.localStorage.getItem(STORAGE_KEYS.SETTINGS_ROOT);
          if (!verification) {
            console.error('❌ [STORAGE] Falha na verificação: dados não foram salvos no localStorage');
          }
        } catch (error) {
      ...
      return {
        ...
        set: (data: UserSettings) => { 
          try {
            ...
            console.error('❌ [STORAGE] Erro ao salvar no electron-store:', error);
          } 
        }
      };
    } catch (error) {
...
export function cleanupStorage(): void {
  ...
  console.log(`🧠 [STORAGE] Limpeza neural-simbólica concluída: ${removedCount} chaves antigas removidas`);
}
